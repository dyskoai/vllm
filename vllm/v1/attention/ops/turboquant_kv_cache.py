...
TURBOQUANT_KV_CACHE_BITS = {"turboquant_3_2": 2.5}
TURBOQUANT_OUTLIER_RATIOS = {"turboquant_3_2": 0.25}
TURBOQUANT_GROUP_BITS = {"turboquant_3_2": (3, 2)}
...
def get_turboquant_packed_dim(head_size: int, kv_cache_dtype: str) -> int:
...
def _structured_signs_cached(
    ...
    if device.type == "cuda":
        with torch.accelerator.device_index(device.index):
            values = torch.randint(0, 2, (dim,), generator=generator, device=device)
    else:
        values = torch.randint(0, 2, (dim,), generator=generator, device=device)
    return torch.where(
        values > 0,
...
def get_turboquant_centroids(device: torch.device, dim: int, bits: int) -> torch.Tensor:
...
def dequantize_turboquant_vectors(
    ...
    output = torch.zeros(
        (*packed.shape[:-2], packed.shape[-2], head_size),
        dtype=torch.float32,
        device=packed.device,
    )
    for group_output, indices in zip(group_outputs, group_indices, strict=True):
        output.scatter_add_(
            -1,
